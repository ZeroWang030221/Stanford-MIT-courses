...
def sim(z_i, z_j):
    ...
    norm_dot_product = z_i @ z_j.T / (torch.linalg.norm(z_i) * torch.linalg.norm(z_j.T, dim=0))
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        l_k = -torch.log((torch.exp(sim(z_k, z_k_N) / tau)) /
                         (torch.sum(torch.exp(sim(z_k, out) / tau)) - torch.exp(sim(z_k, z_k) / tau)))

        l_k_N = -torch.log((torch.exp(sim(z_k_N, z_k) / tau)) /
...
def compute_sim_matrix(out):
    ...
    sim_matrix = ((out @ out.T) /
                  (torch.linalg.norm(out, dim=1, keepdim=True) * torch.linalg.norm(out, dim=1, keepdim=True).T))
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    sim_pl = torch.exp(sim_positive_pairs(out_left, out_right) / tau)
    sim_pr = torch.exp(sim_positive_pairs(out_right, out_left) / tau)
    ...
    numerator = torch.cat([sim_pl, sim_pr], dim=0)
