...
def discriminator_loss(logits_real, logits_fake):
    ...
    true_labels = torch.ones(logits_real.size()[0]).type(dtype)
    false_labels = torch.zeros(logits_fake.size()[0]).type(dtype)
...
def generator_loss(logits_fake):
    ...
    false_labels = torch.ones(logits_fake.size()[0]).type(dtype)
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss_false = torch.mean(scores_fake ** 2)

    loss = 0.5 * (loss_real + loss_false)
