...
def rnn_step_backward(dnext_h, cache):
    ...
    dnext_h = (1 - np.tanh(x @ Wx + prev_h @ Wh + b) ** 2) * dnext_h
    dx = dnext_h @ Wx.T
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for t in range(T):
        h_tmp, cache_tmp = rnn_step_forward(x[:, t, :], h_tmp, Wx, Wh, b)
        h[:, t, :] = h_tmp
...
def word_embedding_backward(dout, cache):
    ...
    D = W.shape[1]
    dW = np.zeros([V, D])
    np.add.at(dW, [idx for idx in x], dout)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dnext_h_tmp = dnext_h * o
    dnext_c = (1 - np.tanh(f * prev_c + i * g) ** 2) * dnext_h_tmp + dnext_c
    dprev_c = f * dnext_c
...
def lstm_backward(dh, cache):
    ...
    T = dh.shape[1]
    H = dh.shape[2]
    x_tmp, _, _, _, _, _, _, _, _, _ = cache[0]
    D = x_tmp.shape[1]
    ...
    for t in range(T - 1, -1, -1):
        if t == (T - 1):
            dx[:, t, :], dprev_h, dprev_c, dWx_tmp, dWh_tmp, db_tmp = lstm_step_backward(dh[:, t, :], dprev_c, cache[t])
        else:
            dx[:, t, :], dprev_h, dprev_c, dWx_tmp, dWh_tmp, db_tmp = lstm_step_backward((dh[:, t, :] + dprev_h),
                                                                                         dprev_c, cache[t])
            if t == 0:
                dh0 = dprev_h
